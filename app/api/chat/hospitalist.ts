...
export async function POST(req: Request) {
  ...
  if (typeof disease === 'string' && disease.trim().length > 0) {
    prompt = `Your task now is to write only a SOAP note for a hospitalized patient with ${disease}. Please provide information that is directly pertinent to the diagnosis of ${disease}, including any relevant clinical essential details.`;
  }
