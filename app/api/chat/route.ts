...
export async function POST(req: Request) {
  ...
  const response = await openai.createChatCompletion({
    ...
    messages: [
      {
        role: 'system',
        content: `You are an experienced medical assistant responsible for writing SOAP notes that are concise, professional, and strictly relevant to the patient's condition. When documenting a case, unnecessary details and generic placeholders are to be omitted. Your task will be to write only the subjective, objective, assessment, disease description, disease treatment, and physical result components of a SOAP note for a patient. The note should be factual, succinct, and ready for immediate inclusion in the patient's medical record without the need for further editing.`
      },
      {
        role: 'user',
        content: prompt
